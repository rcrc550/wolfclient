...
VideoRead::VideoRead(QObject *parent) : QObject(parent),m_tuer(":/images/tuer.png") , m_hat(":/images/hat.png")
{
    ...
    m_shared_worker = QSharedPointer<Videoworkder>(new Videoworkder);
    m_moji=0;
    m_shared_worker.get()->slot_setinfo(this);
    connect( m_timer , SIGNAL(timeout())
             , m_shared_worker.data() , SLOT(slot_dowork())  );
    //需要 xml 加到exe 同级目录里面
...
void VideoRead::slot_getVideoFrame()
{
    ...
    //转化为大小更小的图片  保持缩放比
    image = image.scaled( IMAGE_WIDTH,IMAGE_WIDTH, Qt::KeepAspectRatio );//IMAGE_WIDTH,IMAGE_WIDTH,
...
void VideoRead::slot_setMoji(int i)
{
    m_moji=i;
}
